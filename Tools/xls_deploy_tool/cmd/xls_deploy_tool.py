...
class SheetInterpreter:
    ...
    def Interpreter(self) :
        ...
        self._output.append("package "+FILEHEAD+";\n")
...
class DataParser:
    ...
    def _WriteData2File(self, data) :
        file_name =  self._sheet_name + ".data"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()

    def _WriteReadableData2File(self, data) :
        file_name = self._sheet_name + ".txt"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()
...
if __name__ == '__main__' :
    ...
    b = xlrd.open_workbook(xls_file_path)
    count = len(b.sheets()) #sheet数量
    for sheet in b.sheets():
        if op == 0 or op == 1:
            try :
                tool = SheetInterpreter(xls_file_path, sheet.name)
                tool.Interpreter()
            except BaseException, e :
        ...
        if op == 0 or op == 2:
            try :
                parser = DataParser(xls_file_path, sheet.name)
                print "Parsing --------"
                parser.Parse()
