...
namespace EasyEditor
{
	public class Utils {
        ...
        public static void CreateEditorWindowScript(Type monoType, string path)
        {
            string monoTypeName = monoType.ToString();
            if (monoTypeName.Contains("."))
            {
            ...
            if (!File.Exists(path + "/" + monoTypeName + "EditorWindow.cs"))
            {
                try
                {
                    TextAsset editorScriptPattern = AssetDatabase.LoadAssetAtPath(ResourcesLocation.ResourcesPath + "Implementation/ScriptPattern/EditorWindowScriptPattern.cs.txt", typeof(TextAsset)) as TextAsset;
                    string newEditorScript = string.Copy(editorScriptPattern.text);

                    newEditorScript = newEditorScript.Replace("[UnityObject]", monoTypeName);
                    newEditorScript = newEditorScript.Replace("[naming]", monoTypeName);
                    ...
                    File.WriteAllText(path + "/" + monoTypeName + "EditorWindow.cs", newEditorScript);
                    AssetDatabase.Refresh();
                }
                catch (Exception e)
                {
